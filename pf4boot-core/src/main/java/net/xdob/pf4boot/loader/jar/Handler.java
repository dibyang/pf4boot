...
public class Handler extends URLStreamHandler {
	...
	protected URLConnection openConnection(URL url) throws IOException {
		if (this.jarFile != null && isUrlInJarFile(url, this.jarFile)) {
			return net.xdob.pf4boot.loader.jar.JarURLConnection.get(url, this.jarFile);
		}
		try {
			return net.xdob.pf4boot.loader.jar.JarURLConnection.get(url, getRootJarFileFromUrl(url));
		}
		catch (Exception ex) {
