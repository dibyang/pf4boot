...
提供可选 HTTP 管理入口。该模块可以通过 optional/compileOnly 方式依赖 `pf4boot-jpa` 的公共类型，且所有 JPA reload controller 必须在 `JpaDomainReloadService` Bean 存在时才注册。
...
若后续认为管理模块不应依赖 JPA，可拆出 `pf4boot-jpa-management-starter`，但首个版本不建议新增模块，避免扩大工程复杂度。

### 6.6 `pf4boot-actuator`
